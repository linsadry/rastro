    ...
    <div style={{ minHeight: '100%', background: C.paper }}>
      ...
      <div style={{ padding: '20px 20px 100px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, background: '#fff', borderRadius: 16, padding: '12px 16px' }}>
          <button onClick={prevMonth} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: C.olive }}>‹</button>
          <div style={{ fontFamily: 'Playfair Display', fontSize: 18, color: C.textDark, fontWeight: 700 }}>
            {MONTHS[month]} {year}
          </div>
          <button onClick={nextMonth} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: C.olive }}>›</button>
        </div>

        {/* Weekdays */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', marginBottom: 8 }}>
          {WEEKDAYS.map((d, i) => (
            <div key={i} style={{ textAlign: 'center', fontSize: 11, color: C.textLight, fontWeight: 700, padding: '4px 0' }}>{d}</div>
